...
const createBookingSchema = (services: ServiceItem[]) =>
  z.object({
    ...
    // birth details not needed for class type — but we track selected service outside form
    void services; void ctx;
  });
...
export function BookingForm({
  ...
  const [successData, setSuccessData] = useState<{
    ...
    paymentId: string; userName: string; whatsappUrl: string;
  } | null>(null);
  const [couponCode, setCouponCode] = useState("");
  const [appliedCoupon, setAppliedCoupon] = useState<CouponItem | null>(null);
  const [couponError, setCouponError] = useState<string | null>(null);

  // ── Selected service tracked OUTSIDE the form so it never causes remount ──
  const [activeServiceId, setActiveServiceId] = useState<string>(
    initialServiceId ?? config.services[0]?.id ?? ""
  );

  // Sync when parent changes initialServiceId (e.g. "Book now" from hero)
  useEffect(() => {
    if (initialServiceId) {
  ...
  const selectedService =
    config.services.find((s) => s.id === activeServiceId) ?? config.services[0];
  ...
  const form = useForm<BookingFormValues>({
    ...
    defaultValues: { fullName: "", phoneNumber: "", email: "", dob: "", tob: "", pob: "", message: "" },
  });
  ...
  const priceAfterService = serviceDiscountPct > 0
    ? Math.round(basePrice * (1 - serviceDiscountPct / 100)) : basePrice;
  const couponDiscountPct = appliedCoupon?.discountPercent ?? 0;
  const finalPrice = couponDiscountPct > 0
    ? Math.round(priceAfterService * (1 - couponDiscountPct / 100)) : priceAfterService;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (d.fullName || d.email || d.phoneNumber) {
        form.reset({
          fullName: d.fullName ?? "", phoneNumber: d.phoneNumber ?? "",
          email: d.email ?? "", dob: d.dob ?? "", tob: d.tob ?? "",
          pob: d.pob ?? "", message: d.message ?? "",
        });
  ...
  useEffect(() => {
    const sub = form.watch((values) => {
      if (typeof window === "undefined" || skipDraftRef.current) {
        skipDraftRef.current = false; return;
      }
      if (values.fullName || values.email || values.phoneNumber) {
        window.localStorage.setItem(BOOKING_DRAFT_STORAGE_KEY,
          JSON.stringify({ ...values, serviceId: activeServiceId }));
      }
    });
  ...
  // Validate birth details manually since they're not in the schema conditionally
  const validateBirthDetails = (values: BookingFormValues): string | null => {
    if (!requiresBirthDetails) return null;
    if (!values.dob) return "Date of birth is required.";
    if (!values.tob) return "Time of birth is required.";
    if (!values.pob || values.pob.length < 2) return "Place of birth is required.";
    return null;
  };
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    const birthErr = validateBirthDetails(values);
    if (birthErr) { setStatusMsg(birthErr); return; }
          ...
          handler: async (response) => {
            try {
              ...
              await insertRows("payment_proofs", [{
                customer_name: values.fullName, email: values.email,
                phone_number: values.phoneNumber, service_name: service.name,
                amount: finalPrice, original_amount: basePrice,
              ...
              const lines = [
                ...
                `Email: ${values.email}`,
                ...(requiresBirthDetails ? [``, `*Birth Details*`,
                  `Date of Birth: ${values.dob}`,
                  `Time of Birth: ${values.tob}`,
                  `Place of Birth: ${values.pob}`] : []),
                ...(values.message ? [``, `*Message:* ${values.message}`] : []),
                ``, `_Payment verified. Please reach out to schedule the consultation._`,
              ];
              ...
              form.reset({ fullName: "", phoneNumber: "", email: "", dob: "", tob: "", pob: "", message: "" });
              setSuccessData({ serviceName: service.name, amount: finalPrice,
                paymentId: response.razorpay_payment_id, userName: values.fullName, whatsappUrl });
              setStep("success");
              resolve();
            } catch (e) { reject(e); }
          },
          modal: { ondismiss: () => { setStep("form"); setStatusMsg("Payment cancelled. You can try again."); resolve(); } },
        });
        rzp.open();
      });
    } catch (err) {
  ...
  return (
    <div className="rounded-[2rem] border border-sage/10 bg-white/80 p-5 shadow-glow backdrop-blur sm:p-7">
      ...
      <p className="mt-1 text-sm text-sage/65">
        Fill your details below and pay securely via Razorpay. The astrologer is notified on WhatsApp automatically.
      </p>
        ...
        <div className="mt-4 rounded-[1.5rem] border border-gold/25 bg-gold/8 px-5 py-4">
          ...
          <div className="mt-1 flex items-center justify-between gap-4">
            ...
            <div className="text-right shrink-0">
              ...
              {totalSavings > 0 && (
                <p className="text-xs font-medium text-emerald-600">Save Rs. {totalSavings}</p>
              )}
            </div>
          </div>
        ...
        <div className="mt-5 flex flex-col gap-5">
          <div className="rounded-[2rem] border border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-7 text-center shadow-glow">
            ...
            <p className="mt-1 text-sm text-sage/70">
              Booking confirmed for <strong>{successData.serviceName}</strong>
            </p>
            ...
            <div className="mt-4 rounded-[1.25rem] border border-gold/20 bg-gold/8 px-5 py-4 text-left">
              <p className="text-sm leading-7 text-sage/80">
                May the stars guide you toward clarity and purpose. Our astrologer will connect with you shortly. 🙏
              </p>
        ...
        <form className="mt-5 grid gap-4" onSubmit={onSubmit}>
          ...
          {requiresBirthDetails && (
            <div className="rounded-[1.25rem] border border-sage/10 bg-ivory/60 p-4">
              <p className="mb-3 text-xs uppercase tracking-[0.2em] text-gold font-medium">Birth Details</p>
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div>
                  <RequiredInput label="Place of Birth" placeholder="City / Town"
          ...
          <label className="text-sm font-medium text-sage">
            Additional Message
            <textarea className={`${inputClass} min-h-20 resize-none`}
