...
const FAQS = [
  { q: "Is this a subscription?",
    a: "No. It is a one-time payment. You pay once and get lifetime access." },
  { q: "How do I access premium videos after payment?",
    a: "After your payment is verified by the admin, your account is upgraded to premium. You can then access all videos at /membership/premium." },
  { q: "How long does approval take?",
    a: "Typically within a few hours. You will receive confirmation on WhatsApp." },
  { q: "Can I use the same account for consultations?",
    a: "Membership and consultations are two separate systems. Your Google account is only for membership access." },
  { q: "What payment methods are accepted?",
    a: "UPI, Cards, and Wallets via Razorpay. After payment, upload your screenshot to complete the request." },
];

/** Call server-side API to upsert profile — avoids RLS anon restrictions */
  ...
  return (
    <div className="min-h-screen bg-ivory font-body text-sage">
      ...
      <div className="mx-auto max-w-7xl px-4 pt-6 sm:px-6 lg:px-8">
        <Link href="/" className="text-sm text-sage/60 hover:text-sage transition">← Back to AstroGenZ</Link>
      </div>

      {/* ── Hero ── */}
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-[2.5rem] px-8 py-16 text-center shadow-glow"
          ...
          <div className="relative">
            ...
            {/* Price block */}
            <div className="mt-8 inline-flex flex-col items-center rounded-[1.5rem] border border-white/15 bg-white/10 px-10 py-6 backdrop-blur">
              {loading ? (
                ...
                {profile?.premium ? (
                  ...
                  <>
                    <Link href="/membership/purchase"
                      className="inline-flex items-center gap-2 rounded-full bg-amber-400 px-8 py-3.5 font-bold text-[#3a1005] transition hover:bg-amber-300">
                      <Lock className="h-5 w-5" /> Pay Rs. {price?.toLocaleString("en-IN")} via Razorpay
                    </Link>
                    <p className="text-xs text-white/55">Signed in as {profile.email}</p>
                  </>
                ) : (
                  <>
                    <GoogleButton label="Step 1 — Sign in with Google" />
                    <p className="text-xs text-white/50">After sign-in, click Pay to complete with Razorpay</p>
                  </>
                )}
              </div>
            )}
            {!enabled && (
              <p className="mt-6 text-sm text-white/60">Membership sales are paused. Check back soon.</p>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {BENEFITS.map((b) => (
            <div key={b.title} className="rounded-[1.75rem] border border-sage/10 bg-white/80 p-6 shadow-glow">
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="rounded-[2rem] border border-gold/20 bg-white/80 p-8 text-center shadow-glow">
          ...
          <p className="mt-2 text-sm text-sage/65">Sign in with Google to access your premium library.</p>
          <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            {profile?.premium ? (
              ...
              <div className="flex flex-col items-center gap-2">
                <p className="text-sm text-sage/70">Signed in as <strong>{profile.email}</strong></p>
                <Link href="/membership/purchase"
                  className="inline-flex items-center gap-2 rounded-full bg-sage px-6 py-3 text-sm font-semibold text-ivory transition hover:bg-sage/85">
                  Pay Now — Rs. {price?.toLocaleString("en-IN")} via Razorpay
                </Link>
              </div>
            ) : (
      ...
      <section className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-3">
          {FAQS.map((faq, i) => (
            <div key={i} className="rounded-[1.5rem] border border-sage/10 bg-white/80 overflow-hidden shadow-glow">
              <button type="button"
